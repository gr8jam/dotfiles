...
syn match yamlBlockCollectionItemStart '^\s*\zs-\%(\s\+-\)*\s' nextgroup=yamlBlockMappingKeyName,yamlBlockMappingMerge
...
execute 'syn match yamlBlockMappingKey /\%#=1^\s*\zs'.s:ns_plain_out.'\%(\s\+'.s:ns_plain_out.'\)*\ze\s*:\%(\s\|$\)/ '.
            \'nextgroup=yamlKeyValueDelimiter'
execute 'syn match yamlBlockMappingKeyName /\%#=1\s*\zs'.s:ns_plain_out.'\%(\s\+'.s:ns_plain_out.'\)*\ze\s*:\%(\s\|$\)/ contained '.
            \'nextgroup=yamlKeyValueDelimiterName'


"execute 'syn match yamlBlockMappingKeyAtribute /\%#=1^\s*\zn'.s:ns_plain_out.'\%(\s\+'.s:ns_plain_out.'\)*\ze\s*:\%(\s\|$\)/ '.
"            \'nextgroup=yamlKeyValueDelimiter'
...
syn match   yamlKeyValueDelimiterName /\s*:/ contained nextgroup=yamlKeyValueName

syn region yamlKeyValueName start=/\s*/ end=/$/ contained
